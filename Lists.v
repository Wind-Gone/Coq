...
(** 如果p是由其他form composite的, then we can destruct*)
...
(**
Fixpoint app2 (l1 l2 : natlist) : natlist :=
  match l2 with
  | nil => l1
  | h :: t => app2 (app2 l1 [h]) t
  end.
*)
...
Definition countoddmembers (l:natlist) : nat :=
  match l with
  | nil => 0
  | h :: t => if (oddb h) then (length (oddmembers t) + 1) else length (oddmembers t)
  end.
...
Fixpoint alternate2 (l1 l2 : natlist) : natlist :=
  match l1,l2 with
  | nil, _ => l1
  | _ ,nil => l2
  | h:: t, h' :: t' => h :: h' :: alternate2 t t'
end.
...
Fixpoint replace_one (n m:nat)(l:natlist) : natlist :=
    match l with
    | nil => nil
    | h :: t => if(eqb h n) then m :: t else h :: (replace_one n m (t))
end.
...
Theorem app_nil : forall l : natlist,
  l ++ [] = l.
Proof. intro l. induction l as [| h t IH].
  -reflexivity.
...
Fixpoint eqblist (l1 l2 : natlist) : bool :=
    ...
    | h :: t => match l2 with
            | nil => false
            | h' :: t' => if (eqb h h') then eqblist t t'
                          else false
            end
     end.            
...
Proof.
  ...
  - reflexivity.
  - simpl.  simpl. rewrite -> nat_equals_self. rewrite <- IHn'.
    reflexivity.
Qed.
(** [] *)
