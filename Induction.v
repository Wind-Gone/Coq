...
Proof.
  ...
  - (*n = 0*)reflexivity.
  - (*n = S n'*)simpl. rewrite <- H. reflexivity.
Abort.
...
Theorem plus_swap_help : forall n m p: nat,
  n + (m + p) = n + m + p.
Proof.
  intros n m p.
  induction n as [| n'].
...
Theorem   plus_exchange  :   forall   n   m   p  :   nat,
   n + (m + p) = m + (n + p).
Proof.
  intros n m p.
  rewrite -> plus_comm.
...
Theorem helper : forall n m, m * S n = m + m * n.
Proof.
  intros n m.
  induction m as [|m' H].
    + reflexivity.
    + simpl. rewrite -> plus_exchange. rewrite -> H. reflexivity. Qed.
...
Proof.
    ...
    + simpl. rewrite <- H. rewrite -> helper. reflexivity.
Qed.
...
Fixpoint bin_to_nat (m:bin) : nat :=
  match m with
  | Z => O
  | B0 (t) => plus (bin_to_nat t) (bin_to_nat t)
  | B1 (t) => S (plus (bin_to_nat t) (bin_to_nat t))
  end.
